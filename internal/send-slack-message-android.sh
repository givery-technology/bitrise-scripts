...
if [ ${ex_code} -eq 0 ] ; then
  ...
  ATTACHEMENTS="[{\"fallback\": \"$TASK_NAME: $BITRISE_BUILD_URL\",\"color\": \"$SLACK_COLOR\",\"pretext\": \"$TASK_NAME: <$BITRISE_BUILD_URL|#$BITRISE_BUILD_NUMBER> $GIT_CLONE_COMMIT_MESSAGE_SUBJECT\",\"text\":\"Task: $GRADLE_TASK \n\n$SLACK_ADDITIONAL_MESSAGE<$BITRISE_PUBLIC_INSTALL_PAGE_URL|Install>\"}]"
